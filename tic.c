...
/*
* Tic tac toe AI, courtesy of Alex C Dean (First ever serious C program)
* V2.0
*
*
*/
...
#define LEGAL ((a < 4 && a > 0) && (b < 4 && b > 0) && \ 
               (board[((a-1)*3)+(b-1)] == ' ')) //TODO refactor/remove into inline
...
int checkColumn(char *board){
  int i, sum;
  // For a column checker, we need to do 0 3 6, 1 4 7, 2 5 8. 
  for(i = 0; i < 3; i++){
...
int checkDiag(char *board){
  ...
  //TODO magic numbers, 3x of O/X?
  if((sum = board[2] + board[4] +  board[6]) == 264 ||
    (sum = board[0] + board[4] +  board[8]) == 264){
    return 1;
  }
  
  if(( sum = board[0] + board[4] +  board[8]) == 237 ||
    (sum = board[2] + board[4] +  board[6]) == 237){
    return 2;
  }
...
void AI(char *board){
  ...
  for(i = 0; i < 9; i++){
    if(isLegal(board, i)){
      board[i] = 'O'; // That's us
#ifdef PRINT_AI_MINMAX_STATES
      map(board); //Debugging purposes
#endif
...
int Score(char *temp){
  //If only these magic numbers were documented.....!! TODO figure out
